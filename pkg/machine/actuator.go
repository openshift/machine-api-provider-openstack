...
func (oc *OpenstackClient) convertMachineToCapoInstanceSpec(osc *openStackContext, machine *machinev1.Machine) (*compute.InstanceSpec, error) {
	...
	var createAddressPairs bool = true
	if clusterInfra.Status.PlatformStatus.OpenStack.LoadBalancer != nil && clusterInfra.Status.PlatformStatus.OpenStack.LoadBalancer.Type != "" && clusterInfra.Status.PlatformStatus.OpenStack.LoadBalancer.Type == configv1.LoadBalancerTypeUserManaged {
...
func (oc *OpenstackClient) Delete(ctx context.Context, machine *machinev1.Machine) error {
	...
	err = computeService.DeleteInstance(machine, instanceStatus, machine.Name, rootVolume)
	if err != nil {
		return err
	}
