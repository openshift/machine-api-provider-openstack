...
func networkParamToCapov1PortOpt(net *machinev1alpha1.NetworkParam, apiVIP, ingressVIP string, trunk *bool, networkService *networking.Service, createAddressPairs bool) ([]capov1.PortOpts, error) {
	ports := []capov1.PortOpts{}

	addressPairs := []capov1.AddressPair{}
	if !net.NoAllowedAddressPairs && createAddressPairs {
...
func MachineToInstanceSpec(machine *machinev1beta1.Machine, apiVIP, ingressVIP, userData string, networkService *networking.Service, instanceService *clients.InstanceService, createAddressPairs bool) (*compute.InstanceSpec, error) {
	...
	for _, network := range ps.Networks {
		ports, err := networkParamToCapov1PortOpt(&network, apiVIP, ingressVIP, &ps.Trunk, networkService, createAddressPairs)
		if err != nil {
			return nil, err
		}
